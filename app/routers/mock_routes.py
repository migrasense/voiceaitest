...
"""
CORRECTED TTS Function for Twilio Streaming
Replace your existing synthesize_audio_file() with this version
"""
...
# FIXED: synthesize_audio_file() function for mock_routes.py

# Replace your current synthesize_audio_file() function with this:

async def synthesize_audio_file(text: str, language: str = "en") -> bytes:
    """
    Convert text to speech using Deepgram API.
    Returns mu-law encoded audio bytes suitable for Twilio.
    """
    try:
        logger.info(f"🎤 Generating TTS with aura-asteria-en: {text[:50]}...")
        
        from deepgram import SpeakOptions
        import io
        
        # Create options with correct encoding for Twilio (mu-law)
        options = SpeakOptions(
            model="aura-asteria-en",
            encoding="mulaw",      # ✅ CRITICAL: Twilio uses mu-law
            sample_rate=8000       # ✅ CRITICAL: Twilio uses 8kHz
        )
        
        # Create buffer to capture audio
        buffer = io.BytesIO()
        
        logger.info(f"🎤 Requesting speech synthesis from Deepgram...")
        ...
        if not audio_data or len(audio_data) == 0:
            logger.error(f"❌ Deepgram returned empty audio for text: {text}")
            return None
...
# ALTERNATIVE: If the above doesn't work, use REST API directly

async def synthesize_audio_file_rest(text: str, language: str = "en") -> bytes:
    """
    Alternative: Use Deepgram REST API directly instead of SDK stream.
    Use this if the stream() method isn't working.
    """
    try:
        import os
        import requests
        
        logger.info(f"🎤 Generating TTS via REST API: {text[:50]}...")
        ...
        logger.info(f"🌐 POSTing to Deepgram: {url}")
        ...
        logger.info(f"📤 Deepgram response: {response.status_code}")
        ...
        if not audio_data or len(audio_data) == 0:
            logger.error(f"❌ Response body is empty")
            return None
        
        return audio_data
        
    except Exception as e:
        logger.error(f"❌ REST API TTS failed: {e}", exc_info=True)
        return None


# TEST FUNCTION: Verify TTS is working

async def test_tts():
    """Quick test to verify TTS is generating audio"""
    try:
        logger.info("🧪 Testing TTS generation...")
        
        audio = await synthesize_audio_file("Hello, this is a test", "en")
        
        if audio and len(audio) > 0:
            logger.info(f"✅ TTS TEST PASSED: Generated {len(audio)} bytes")
            return True
        else:
            logger.error(f"❌ TTS TEST FAILED: No audio returned")
            return False
...
# After (no change needed - just use the fixed function above)


# ============================================================
# OPTIONAL: Legacy function for reference (save to disk)
# Only use if you need to store audio files
# ============================================================

async def synthesize_audio_file_legacy(text: str, language: str = "en") -> Optional[str]:
    """
    [LEGACY] Generate TTS and save to disk.
    
    WARNING: Use synthesize_audio_file() instead for Twilio streaming.
    ...
    Returns:
        Audio file ID (filename) or None if failed
    """
    if not text.strip():
        return None
    ...
    try:
        ...
        model = "aura-asteria-es" if language == "es" else "aura-asteria-en"
        
        options = SpeakOptions(
            model=model,
            encoding="mulaw",        # ✅ Still use mu-law
            sample_rate=8000,        # ✅ Still use 8kHz
            container="wav"
        )
        
        deepgram.speak.v("1").save(filepath, {"text": text}, options)
        
        if os.path.exists(filepath) and os.path.getsize(filepath) > 0:
            logger.info(f"📁 TTS file saved: {filepath} ({os.path.getsize(filepath)} bytes)")
            return audio_id
        else:
    ...
    if new_session_available:
        try:
            ...
            if not phone_lookup.data:
                logger.warning(f"⚠️ No phone match found for {phone_number}, skipping Supabase flush.")
                return {
...
def test_prompt(config: dict):
    ...
    try:
        ...
        prompt_data = {
            ...
            "services_description": config.get("services_description", ""),
            "tone": config.get("tone", "Professional"),
            "role": config.get("role", "Receptionist"),
            "urgency": config.get("default_urgency", "Normal")
        }

        print("🟢 Prompt Data:", prompt_data)

        system_prompt = groq_client._create_natural_system_prompt(
            language="en",
            is_first_turn=True,
            company_data=prompt_data
        )

        print("✅ Generated Prompt OK")
